...
class ExperimentProfiler:
    ...
    def print_report(self):
        ...
        if self.metrics['iterations_completed'] > 0:
            print(f"  Seconds per Iteration: {self.metrics['duration_seconds'] / self.metrics['iterations_completed']:.2f}s")
        ...
        if self.metrics['duration_seconds'] > 0:
            print(f"  Requests per Second:   {self.metrics['requests_processed'] / self.metrics['duration_seconds']:.2f}")
